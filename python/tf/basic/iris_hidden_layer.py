...
saver = Saver('iris_hidden_layer')


def _hidden_layer1(x):
    W1 = tf.Variable(tf.truncated_normal((4, 20), stddev=0.1))
    b1 = tf.Variable(tf.zeros((20,)))
    return tf.nn.tanh(tf.matmul(x, W1) + b1)


def _hidden_layer2(h1: tf.Tensor):
    W2 = tf.Variable(tf.truncated_normal((20, 10), stddev=0.1))
    b2 = tf.Variable(tf.zeros((10,)))
    return tf.nn.tanh(tf.matmul(h1, W2) + b2)


def _softmax(h2: tf.Tensor) -> tf.Tensor:
    W3 = tf.Variable(tf.truncated_normal((10, 3), stddev=0.1))
    b3 = tf.Variable(tf.zeros((3,)))
    return tf.nn.softmax(tf.matmul(h2, W3) + b3, name='softmax')
